...
namespace KiraExtraPieces.Parts
{
    public class PartBeam1x4 : Part
    {
        ...
        public override PartType PartType => (PartType)ExtraPartType.PartBeam1x4;

        public override bool GeneratesDownforce => false;

        public override string TooltipText => "";

        public override string PartName => "Beam x4";
        ...
        private ConnectionpointData[] datas =
        {
        ...
        private (PartDirection, PartRotation, Connectionpoint[]) connectionsCache;

        public override Connectionpoint[] GetConnectionpoints(PartDirection direction, PartRotation rotation)
        {
            (PartDirection directionCache, PartRotation rotationCache, Connectionpoint[] connectionpointsCache) = connectionsCache;
            if(directionCache.Equals(direction) && rotationCache.Equals(rotation))
            {
                return connectionpointsCache;
            }

            Vector3Int[] universalDirections = PartUtils.GetUniversalDirections(direction, rotation);
            ...
            Connectionpoint[] array = ConnectionpointUtils.MakeConnectionPoints(datas, uD);

            //Connectionpoint[] array = new Connectionpoint[]
            //{
            //    ExtraPieces.PartMapping.Utils.MakeConnectionPoint<ConnectorRoundHole>(partDirection, uD, new Vector3(0f, 0f, 0f)),
            //    ExtraPieces.PartMapping.Utils.MakeConnectionPoint<ConnectorRoundHole>(partDirection, uD, new Vector3(0f, 0f, 1f)),
            //    ExtraPieces.PartMapping.Utils.MakeConnectionPoint<ConnectorRoundHole>(partDirection, uD, new Vector3(0f, 0f, 2f)),
            //    ExtraPieces.PartMapping.Utils.MakeConnectionPoint<ConnectorRoundHole>(partDirection, uD, new Vector3(0f, 0f, 3f)),
            //};

            connectionsCache = (direction, rotation, array);
